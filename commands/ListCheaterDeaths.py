...
class ListCheaterDeaths(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.hybrid_command(
        name="list_cheater_deaths", description="List all reported cheater deaths."
    )
    ...
    async def list_cheater_deaths(self, ctx):
        ...
        async def get_page(page):
            ...
            embed = discord.Embed(
                title="Cheater Death Reports", color=discord.Color.red()
            )
...
async def setup(bot):
    await bot.add_cog(ListCheaterDeaths(bot))
